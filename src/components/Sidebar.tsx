  ...
  return <motion.div layout initial={{
    ...
    x: {
      ...
      scale: 0.95
        ...
        <MessageSquare className={`w-4 h-4 lg:w-5 lg:h-5 transition-all duration-200 ${isChatPanelOpen ? 'text-indigo-600' : 'text-slate-500 group-hover:text-indigo-600'}`} strokeWidth={1.5} />
      </motion.button>
        ...
        return <motion.button key={item.id} initial={{
          ...
          handleItemClick(item.id);
            ...
            scaleY: 1
              ...
              {/* Icon */}
              <Icon className={`w-4 h-4 lg:w-5 lg:h-5 transition-all duration-200 ${isActive ? 'text-indigo-600' : 'text-slate-500 group-hover:text-indigo-600'}`} strokeWidth={1.5} />
