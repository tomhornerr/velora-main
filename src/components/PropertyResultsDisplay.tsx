...
export default function PropertyResultsDisplay({
  ...
  return <div className={`w-full max-w-4xl mx-auto px-4 py-6 ${className || ''}`}>
      ...
      <div className="flex items-center justify-center space-x-6 mt-4">
        {/* Left Arrow */}
        <motion.button onClick={prevProperty} className="w-12 h-12 bg-white/90 backdrop-blur-sm hover:bg-white shadow-lg border border-white/20 rounded-full flex items-center justify-center transition-all duration-200" whileHover={{
        scale: 1.1,
        x: -2
      }} whileTap={{
        scale: 0.9
        ...
        <div className="flex justify-center space-x-2">
          {properties.map((_, index) => <motion.button key={index} onClick={() => goToProperty(index)} className={`h-2 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-gradient-to-r from-emerald-500 to-teal-500 w-6 shadow-lg' : 'bg-slate-300 hover:bg-slate-400 w-2'}`} whileHover={{
          scale: 1.2
        }} whileTap={{
          scale: 0.8
        ...
        {/* Right Arrow */}
        <motion.button onClick={nextProperty} className="w-12 h-12 bg-white/90 backdrop-blur-sm hover:bg-white shadow-lg border border-white/20 rounded-full flex items-center justify-center transition-all duration-200" whileHover={{
        scale: 1.1,
        x: 2
      }} whileTap={{
        scale: 0.9
